...
export default function KpiCard({ label, value, suffix = "", decimals = 0, delta, deltaDir = "flat", color = "var(--brand)", Icon, index = 0 }) {
  const animated = useCountUp(typeof value === "number" ? value : 0);
  const shown = typeof value === "number"
    ? animated.toFixed(decimals)
    : value;
  return (
    <div className="kpi" style={{ animationDelay: `${index * 0.06}s` }}>
